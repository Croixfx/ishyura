...
function Index() {
  ...
  const handleGoogleSignIn = async (emailOverride?: string, nameOverride?: string) => {
    const emailToUse = (emailOverride || googleEmailInput || "merchant@kigalibusiness.rw").trim();
    ...
    try {
      const user = await IshyuraClient.signInWithGoogle({
        email: emailToUse,
        name: nameOverride || googleNameInput || businessName || "Merchant",
      });
      setCurrentUser(user);
      setAuthDialogOpen(false);
      setGoogleEmailInput("");
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const email = prompt(
                            ...
                            googleEmailInput || "merchant@kigalibusiness.rw",
                          );
                          if (email) {
                            handleGoogleSignIn(email);
                          }
                        }}
                        className="w-full h-11 text-xs font-semibold gap-2.5 border-border/80 bg-background hover:bg-muted/60 shadow-xs"
                      >
            ...
            {!currentUser ? (
              <div className="mt-6 rounded-2xl border border-primary/30 bg-primary/5 p-4 sm:p-5">
                ...
                <p className="mt-1.5 text-xs text-muted-foreground leading-relaxed">
                  Sign in with your phone number to generate and permanently own your payment QR
                  tent cards across all your devices.
                </p>
                <Button
                  type="button"
                  size="sm"
                  ...
                  className="mt-3 w-full text-xs font-semibold gap-1.5 shadow-sm"
                >
                  <LogIn className="size-3.5" />
